...
bool layer_ctor(struct layer_s *layer, char *name, void *user_data, layer_on_attach_f on_attach,
...
void layer_update(struct layer_s *layer, float dt);
void layer_gui(struct layer_s *layer);
bool layer_event(struct layer_s *layer, event_s *event);
