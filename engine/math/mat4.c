...
mat4 mat4_scale(mat4 m, float by) {
  ...
  return mat4_new(m.xx * by, m.xy * by, m.xz * by, m.xw * by,
      m.yx * by, m.yy * by, m.yz * by, m.yw * by,
      m.zx * by, m.zy * by, m.zz * by, m.zw * by,
      m.tx * by, m.ty * by, m.tz * by, m.tw * by);
...
mat4 mat4_mul(mat4 a, mat4 b) {
  ...
  return mat4_new(M4D(a,0,b,0), M4D(a,1,b,0), M4D(a,2,b,0), M4D(a,3,b,0),
...
vec4 mat4_multiply_vec4(const mat4 m, const vec4 v) {
    return vec4_new(M4V4D(m, 0, v.x, v.y, v.z, v.w),
...
vec3 mat4_transform_vec3(mat4 m, vec3 v) {
  ...
  return vec3_new(M4V4D(m, 0, v.x, v.y, v.z, 0.0f),
...
vec3 mat4_transform_point(mat4 m, vec3 v) {
  ...
  return vec3_new(M4V4D(m, 0, v.x, v.y, v.z, 1.0f),
...
mat4 mat4_perspective(float fovDegrees,
                        float aspect,
                        float near,
                        float far)
{
...
mat4 mat4_look_at(vec3 position, vec3 target, vec3 up) {
  ...
  vec3 t = vec3_new(-vec3_dot(r, position), -vec3_dot(u, position),
                    -vec3_dot(f, position));
