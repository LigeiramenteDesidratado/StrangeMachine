...
typedef enum {
  ...
  BODY_TYPE_MAX = (SPHERE | CAPSULE | MESH),
} body_e;
...
/* Believe me, this is a hash table. The Big O is O(1) */
static void (*handler[BODY_TYPE_MAX])(physics_s *physics, physics_s *against) = {
...
void physics_step(void) {
  ...
  for (signed char i = 0; i <= entity.count; ++i) {
    for (signed char j = i + 1; j <= entity.count; ++j) {
      /* convert the enitity_type into an array index (our key). Kind of a hash function */
