...
b8 application_on_event(event_s *event, void *user_data);
b8 application_on_window_close(event_s *event, void *user_data);
void application_push_overlay(application_s *app, struct layer_s *layer);

b8 application_ctor(application_s *app, const char *name) {
...
b8 application_on_event(event_s *event, void *user_data) {
  ...
  SM_LOG_TRACE("event handled: %d", event->handled);
...
void application_do(application_s *app) {
  ...
  while (app->is_running) {
    ...
    camera_do((f32)app->delta);

    size_t stack_size = stack_layer_get_size(app->stack);
    for (size_t i = 0; i < stack_size; ++i) {
      struct layer_s *layer = stack_layer_get_layer(app->stack, i);
      layer_update(layer, (f32)app->delta * app->thickness);
    }
    ...
    if (input_scan_key_lock(sm_key_q))
