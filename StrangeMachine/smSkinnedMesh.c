...
void skinned_mesh_dtor(skinned_mesh_s *mesh) {
  ...
  SM_ALIGNED_ARRAY_DTOR(mesh->weights);
...
void skinned_mesh_update_gl_buffers(skinned_mesh_s *mesh) {
  ...
  if (SM_ALIGNED_ARRAY_SIZE(mesh->weights) > 0)
    attribute_set(&mesh->weight_attr, mesh->weights, SM_ALIGNED_ARRAY_SIZE(mesh->weights), GL_STREAM_DRAW);
